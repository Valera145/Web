...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
  ...
  if (!$error and !empty($_COOKIE[session_name()]) and !empty($_SESSION['login'])) {
    ...
    try{
      ...
      for($i=0;$i<count($inf2);$i++){
        ...
        if($inf2[$i]['power']=='Телепортация'){
          $values['teleport']=1;
        }
        if($inf2[$i]['power']=='Телепатия'){
          $values['telepat']=1;
        }
      }
    }
  ...
  if(!empty($_POST['logout'])){
    ...
    if (!empty($_COOKIE[session_name()]) && !empty($_SESSION['login']) and !$errors) {
      $id=$_SESSION['uid'];
      $upd=$db->prepare("update application set name=?,mail=?,year=?,sex=?,limb=?,bio=? where id=?");
      $upd->execute(array($name,$email,$year,$sex,$limb,$bio,$id));
      $del=$db->prepare("delete from powers where id=?");
      $del->execute(array($id));
      $upd1=$db->prepare("insert into powers set power=?,id=?");
      foreach($pwrs as $pwr){
      ...
      if(!$errors){
        ...
        try {
          $stmt = $db->prepare("INSERT INTO application SET name=?,mail=?,year=?,sex=?,limb=?,bio=?");
          $stmt -> execute(array($name,$email,$year,$sex,$limb,$bio));
          $id=$db->lastInsertId();
          $pwr=$db->prepare("INSERT INTO powers SET power=?,id=?");
