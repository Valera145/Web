...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
  ...
  try{
      ...
      for($i=0;$i<count($powers);$i++){
        if($powers[$i]['power']=='Бессмертие'){
          $values['immortal']=1;
        }
        if($powers[$i]['power']=='Телепортация'){
          $values['teleport']=1;
        }
        if($powers[$i]['power']=='Телепатия'){
          $values['telepat']=1;
        }
      }
  }
  ...
  if(!empty($_POST['edit'])){
    ...
    if(!$errors){
        ...
        $upd=$db->prepare("insert into powers set power=?,id=?");
        foreach($pwrs as $pwr){
